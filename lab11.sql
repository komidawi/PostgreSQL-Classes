...
CREATE OR REPLACE FUNCTION pobierz_zamowienia_klienta(idklienta INTEGER)
   ...
   $$
      ...
      BEGIN
         FOR wiersz IN
            SELECT * FROM pobierz_ID_oraz_daty_zamowien(idklienta)
         LOOP
            RETURN QUERY
...
--Rabat wyliczany jest na podstawie zamówień bieżących (tabela zamowienia)
...
CREATE OR REPLACE FUNCTION rabat(idklienta_ TEXT)
RETURNS NUMERIC(5,2) AS
   $$
      DECLARE
         wysokoscRabatu NUMERIC(5,2);
         kwotaZamowien NUMERIC(7,2);
      BEGIN
         kwotaZamowien := zsumuj_calkowita_kwote_zamowien(idklienta_);

         IF kwotaZamowien = 0 THEN
            wysokoscRabatu := 0.00;
         ELSIF kwotaZamowien > 0 AND kwotaZamowien <= 100 THEN
            wysokoscRabatu := 0.05;
         ELSIF kwotaZamowien > 100 AND kwotaZamowien <= 400 THEN
            wysokoscRabatu := 0.10;
         ELSIF kwotaZamowien > 400 AND kwotaZamowien <= 700 THEN
            wysokoscRabatu := 0.15;
         ELSE
            wysokoscRabatu := 0.20;
         END IF;
...
CREATE OR REPLACE FUNCTION zsumuj_calkowita_kwote_zamowien(idklienta_ TEXT)
RETURNS NUMERIC(7,2) AS
   $$
      DECLARE
         calkowitaKwotaZamowien NUMERIC(7,2);
         kwotaZamowienBiezacych NUMERIC(7,2);
         kwotaZamowenHistorycznych NUMERIC(7,2);
      BEGIN
         kwotaZamowienBiezacych := pobierz_kwote_zamowien_biezacych(idklienta_);
         kwotaZamowenHistorycznych := pobierz_kwote_zamowien_historycznych(idklienta_);
         calkowitaKwotaZamowien := kwotaZamowienBiezacych + kwotaZamowenHistorycznych;

         RETURN calkowitaKwotaZamowien;
      END;
   $$
LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION pobierz_kwote_zamowien_biezacych(idklienta_ TEXT)
RETURNS NUMERIC(7,2) AS
   $$
      BEGIN
         RETURN (SELECT sum(cena) FROM zamowienia WHERE idklienta = idklienta_);
      END;
   $$
LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION pobierz_kwote_zamowien_historycznych(idklienta_ TEXT)
RETURNS NUMERIC(7,2) AS
   $$
      BEGIN
         RETURN (SELECT sum(cena) FROM historia WHERE idklienta = idklienta_);
      END;
   $$
LANGUAGE plpgsql;
